...
Thanks to the central limit theorem, we can assume that $\theta^*$ is distributed normally under large sample sizes. The mean of this distribution is found to be $\theta^*$, but our covariance matrix is a little trickier to find. Since we don't know exactly the structure of $\Sigma^*$, we can estimate it using the Fisher Information matrix, which is given below:
...
\section*{Problem 2.5}
Using H that we defined above, we can show the distribution of $\hat{y}$ below:
